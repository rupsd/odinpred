...
##for d in range(1,11):
 for d in range(10):
   ...
   zscorespred = new_model.predict(x_val2)
   zscorespred[zscorespred>1116.5]=1116.5
   zscorespred[zscorespred<-5]=-5
   pred=np.hstack([pred, zscorespred]) if pred.size else zscorespred
...
plt.title('Disorder predictions for'+' ' +str(a))
